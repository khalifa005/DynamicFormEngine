...
description: >-
  ...
  tables (server sort/filter — see also primeng-table skill), and the Aura/Lara
  theme system with design tokens. Activates when working with primeng components.
...
| Use PrimeNG Table with Lazy Loading | HIGH | Server-side page/sort/filter via onLazyLoad (see skill `primeng-table`) |
| Append Overlays to Body in Dialogs | HIGH | `appendTo="body"` on selects; `paginatorDropdownAppendTo="body"` on tables in `p-dialog` (see skill `primeng-overlays`) |
