...
**Impact: HIGH** (Required for every client API call)

Every client API call must show a busy state. Use skeleton/spinner for page loads. For submit/action buttons that call an API, show a loader and disable the control until the request finishes (PrimeNG: `[loading]` + `[disabled]`). Clear loading in `finalize`/`finally`. Always show helpful `@empty` UI for zero-data states.
